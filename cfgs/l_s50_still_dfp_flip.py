...
class Exp(MyExp):
    def __init__(self):
        ...
        self.data_dir = os.environ.get("STREAMYOLO_ARGOVERSE_ROOT", "/data")

        self.exp_name = os.path.split(os.path.realpath(__file__))[1].split(".")[0]

        repo_root = os.path.dirname(os.path.dirname(os.path.realpath(__file__)))
        self.output_dir = os.path.join(repo_root, 'outputs', 'streamyolo_argoverse')

    def get_model(self):
        from exps.model.yolox import YOLOX
        from exps.model.dfp_pafpn import DFPPAFPN
        from exps.model.pipe_head import PIPEHead
        import torch.nn as nn
        ...
        if getattr(self, "model", None) is None:
            ...
            head = PIPEHead(self.num_classes, self.width, in_channels=in_channels)
    ...
    def get_data_loader(self, batch_size, is_distributed, no_aug=False, local_rank=0, cache_img=False):
        ...
        from exps.data.data_augment_flip import TrainTransform
        ...
        dataset = STILL_ARGOVERSEDataset(
            ...
            preproc=TrainTransform(max_labels=50, hsv=False, flip=True),
            cache=cache_img,
        )
        ...
        dataset = MosaicDetection(dataset,
                                  mosaic=not no_aug,
                                  img_size=self.input_size,
                                  preproc=TrainTransform(max_labels=120, hsv=False, flip=True),
    ...
    def get_evaluator(self, batch_size, is_distributed, testdev=False):
        from exps.evaluators.still_stream_evaluator import STILL_COCOEvaluator

        val_loader = self.get_eval_loader(batch_size, is_distributed, testdev)
        evaluator = STILL_COCOEvaluator(
